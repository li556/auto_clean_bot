...
void MiiviiGmslCamera::DeclareParameters() {
    ...
    for (int i = 0; i < this->camera_num; i++) {
        ...
        parameters[i] = {{dev_node + ".node_name", rclcpp::ParameterValue("/dev/" + dev_node)},
...
void MiiviiGmslCamera::CreateCtx() {
    for (int i = 0; i < camera_num; i++) {
        ...
        if (active) {
            ...
            camera_info_publisher_[active_camera_num] =
            ...
            RCLCPP_INFO(this->get_logger(), "Camera Format(camera_fmt) is : %s",
                        ctx[active_camera_num].camera_fmt_str.c_str());
            RCLCPP_INFO(this->get_logger(), "Camera Resolution(camera_resolution) is : %s", camera_resolution.c_str());
            RCLCPP_INFO(this->get_logger(), "Output Format(output_fmt) is: %s",
                        ctx[active_camera_num].output_fmt_str.c_str());
...
void MiiviiGmslCamera::StartTimer() {
    ...
    if (this->publish_rate > 0) {
        ...
        image_publisher_timer_ =
            this->create_wall_timer(publish_period, std::bind(&MiiviiGmslCamera::timer_callback, this));
    }
}
void MiiviiGmslCamera::timer_callback() {
    ...
    if (this->last_time == rclcpp::Time(0)) {
        ...
        // if (static_cast<uint64_t>(jitter.nanoseconds()) > (time_interval + 5000000) ||
        // static_cast<uint64_t>(jitter.nanoseconds()) < (time_interval - 5000000))
        // {
