...
int StageClientDeliver::process_job_(InnoCommonHeader *pkt, bool prefer) {
  ...
  lidar_->do_recorder_callback(INNO_RECORDER_CALLBACK_TYPE_RAW, reinterpret_cast<char *>(pkt), n);
  if (pkt->version.magic_number == kInnoMagicNumberStatusPacket) {
    ...
    if (n >= sizeof(InnoDataPacket)) {
      if (CHECK_SPHERE_POINTCLOUD_DATA(data_packet->type) || CHECK_XYZ_POINTCLOUD_DATA(data_packet->type)) {
        ...
        // get anglehv_table
        if (data_packet->type == INNO_ROBINW_ITEM_TYPE_COMPACT_POINTCLOUD && !lidar_->anglehv_init_) {
            char *table = new char[sizeof(InnoDataPacket) + sizeof(InnoAngleHVTable)];
        ...
        if (lidar_->data_packet_callback_) {
          ...
          if (lidar_->force_xyz_pointcloud_ && CHECK_SPHERE_POINTCLOUD_DATA(data_packet->type)) {
            ...
            if (InnoDataPacketUtils::convert_to_xyz_pointcloud(
                   ...
                   reinterpret_cast<char *>(lidar_->anglehv_table_))) {
        ...
        if (n != data_packet->item_size + sizeof(InnoDataPacket) || data_packet->item_size <= sizeof(InnoMessage) ||
          ...
          if (m->code == INNO_MESSAGE_CODE_NEW_START) {
            ...
            if (!lidar_->time_sync_set_value.empty()) {
              lidar_->set_attribute_string("time_sync_check", lidar_->time_sync_set_value.c_str());
            }
            if (!lidar_->frame_sync_time_.empty()) {
              lidar_->set_attribute_string("server_frame_sync", lidar_->frame_sync_time_.c_str());
            }
      ...
      } else if (data_packet->type == INNO_ROBINW_ITEM_TYPE_ANGLEHV_TABLE) {
        // todo anglehv_table is receive by fragment
        if (n == sizeof(InnoDataPacket) + sizeof(InnoAngleHVTable) && lidar_->anglehv_init_ == false) {
          lidar_->anglehv_init_ = true;
          memcpy(lidar_->anglehv_table_,
                 reinterpret_cast<char *>(
                     reinterpret_cast<InnoAngleHVTable *>((reinterpret_cast<InnoDataPacket *>(data_packet)->payload))
                         ->table),
                 sizeof(lidar_->anglehv_table_));
          inno_log_info("find anglehv_table successful %u", data_packet->common.size);
        }
      } else {
