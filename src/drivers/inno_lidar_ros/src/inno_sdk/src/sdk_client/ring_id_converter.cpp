...
RingIdConverter::RingIdConverter(InnoLidarClient *lidar) {
  lidar_ = lidar;
  init_success_ = (setup_ring_id_table_normal_() == 0 && setup_ring_id_table_quiet_() == 0);
}
...
int RingIdConverter::update_ring_id_table() {
  ...
  char buffer[20] = "\0";
  int ret = lidar_->get_attribute_string("galvo_first_period", buffer, sizeof(buffer));
  if (ret) {
  ...
  try {
    ...
    inno_log_warning("std::invalid_argument::what(): %s, buffer:%s", ex.what(), buffer);
    return -1;
  } catch (std::out_of_range const &ex) {
    inno_log_warning("std::out_of_range::what(): %s, buffer:%s", ex.what(), buffer);
    return -1;
  }
  ...
  uint16_t first_roi_line_up = floor((static_cast<double>(galvo_periods_[0]) / kGalvoPeridUnitsPerSecond) /
                                     single_line_scan_time_s_);
  uint16_t first_roi_line_down = floor((1.0 / frame_rate_ - static_cast<double>(galvo_periods_[0] + galvo_periods_[1] +
                                                                                galvo_periods_[2] + galvo_periods_[3]) /
                                                                kGalvoPeridUnitsPerSecond) /
                                       single_line_scan_time_s_);
  if (first_roi_line_up == current_first_roi_line_up_ && first_roi_line_down == current_first_roi_line_down_) {
  ...
  inno_log_trace(
      "line_num_per_frame=%d, "
      "single_line_scan_time_s_ = %lf, "
      "1st_roi_up=%u, 1st_roi_down=%u, "
      "galvo_periods=%d/%d/%d/%d",
      line_num_per_frame_, single_line_scan_time_s_, first_roi_line_up, first_roi_line_down, galvo_periods_[0],
      galvo_periods_[1], galvo_periods_[2], galvo_periods_[3]);
  fill_table_(first_roi_line_up, first_roi_line_down, line_num_per_frame_, ring_id_table_[1]);
  return 0;
}
...
int RingIdConverter::setup_ring_id_table_normal_() {
  ...
  // get motor config
  char buffer[1024] = "\0";
  int ret = lidar_->get_attribute_string("internal_motor_config", buffer, sizeof(buffer));
  if (ret) {
  ...
  ret = get_config_map_(buffer, &map);
  if (ret) {
    inno_log_error("get motor config map failed.");
    return ret;
  }
  ...
  inno_log_verify(mtr_f_rpm != 0 && mtr_f_rpm <= kMaxRPM, "invalid mtr_f_rpm: %d", mtr_f_rpm);
  frame_rate_ = 0;
  frame_rate_ = map["galvo_framerate"];
  inno_log_verify(frame_rate_ > 0, "invalid frame rate: %d", frame_rate_);
  single_line_scan_time_s_ = 60.0 / mtr_f_rpm / kPolygonFacet;
  line_num_per_frame_ = kPolygonFacet * mtr_f_rpm / 60 / frame_rate_;
...
int RingIdConverter::setup_ring_id_table_quiet_() {
  ...
  char buffer[1024] = "\0";
  int ret = lidar_->get_attribute_string("internal_quiet_config", buffer, sizeof(buffer));
  if (ret) {
    inno_log_error("get motor config error");
    return ret;
  }
  ...
  uint32_t line_num_per_frame = kPolygonFacet * mtr_f_rpm / 60 / frame_rate;

  fill_table_(UINT16_MAX, UINT16_MAX, line_num_per_frame, ring_id_table_[0]);
  return 0;
}

void RingIdConverter::fill_table_(
    uint16_t first_roi_line_up, uint16_t first_roi_line_down, uint16_t line_num_per_frame,
  ...
  std::vector<ScanLineVirticalAngle> line_angles(line_num_per_frame * kInnoChannelNumber);
  ...
  for (uint32_t scan_direct = 0; scan_direct < INNO_FRAME_DIRECTION_MAX; ++scan_direct) {
    for (uint16_t scan_line = 0, idx = 0; scan_line < line_num_per_frame; ++scan_line) {
      for (uint8_t ch = 0; ch < kInnoChannelNumber; ++ch, idx++) {
        line_angles[idx].scan_line = scan_line;
        line_angles[idx].ch = ch;
        line_angles[idx].v_angle = get_v_angle_of_scanline_(scan_line, ch, first_roi_line[scan_direct]);
        inno_log_trace("set line_angles[%d] to \tsc=%d\tch=%d\tv_angle=%lf", idx, scan_line, ch,
    ...
    std::sort(line_angles.begin(), line_angles.end(),
              [](const ScanLineVirticalAngle &a, const ScanLineVirticalAngle &b) { return a.v_angle < b.v_angle; });
    ...
    for (uint16_t i = 0; i < line_num_per_frame * kInnoChannelNumber; ++i) {
      ...
      uint16_t scan_line = scan_direct ? line_angles[i].scan_line : (line_num_per_frame - line_angles[i].scan_line - 1);
      inno_log_trace(
          "will set ring_id_table_[%d][%d][%d]=%d,"
          "from line_angles[%d], v_angle=%lf",
          scan_direct, scan_line, line_angles[i].ch, ring_id, i, line_angles[i].v_angle);
  ...
  if (inno_log_level_g >= INNO_LOG_LEVEL_TRACE) {
    for (int i = 0; i < 2; ++i) {
      ...
      ss << i << "-";
      for (uint32_t j = 0; j < line_num_per_frame; ++j) {
...
double RingIdConverter::get_v_angle_of_scanline_(uint32_t scanline, uint32_t ch, uint32_t first_roi_line) const {
  uint32_t x = scanline;
...
int RingIdConverter::get_config_map_(const char *buffer, std::unordered_map<std::string, int32_t> *map_out) {
  if (!buffer || !map_out) {
    inno_log_error("invalid argument: buffer=%p, map_out=%p", buffer, map_out);
    return -1;
  }
  std::ostringstream ss;
  ss << buffer;
  ...
  for (const std::string &item : items) {
    ...
    if (key_value.size() == 2) {
      try {
        map_out->emplace(key_value[0], std::stoi(key_value[1]));
      } catch (std::invalid_argument const &ex) {
        inno_log_warning("std::invalid_argument::what(): %s, value:%s", ex.what(), key_value[1].c_str());
        return -1;
      } catch (std::out_of_range const &ex) {
        inno_log_warning("std::out_of_range::what(): %s, value:%s", ex.what(), key_value[1].c_str());
        return -1;
      }
