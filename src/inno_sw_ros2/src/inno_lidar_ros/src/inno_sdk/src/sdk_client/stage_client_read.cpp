...
#else
    inno_log_error("invalid type %d, pcap file input not support.", source_);
    return false;
#endif
  } else {
...
void StageClientRead::add_deliver_packet_(InnoCommonHeader *header) {
  lidar_->do_recorder_callback(INNO_RECORDER_CALLBACK_TYPE_RAW, reinterpret_cast<const char *>(header), header->size);
...
int FileInput::keep_reading_() {
  ...
  while (1) {
    if (!start_flag_) {
      ...
      int major_version = 0;
      int r =
          read_packet_(data_packet, &data_len, message_packet, &message_len, status_packet, &status_len, major_version);
      if (r > 0) {
        read_so_far += r;
        if (data_len) {
          data_cnt++;
...
void FileInput::set_file_play_rate() {
  ...
  inno_log_info("Setting play rate to %d", tmp_rate);
  if (tmp_rate > 100) {
    play_rate_ = 0;
