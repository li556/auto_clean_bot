...
typedef DEFINE_INNO_COMPACT_STRUCT(InnoDataPacketV1) {
  ...
  uint16_t reserved_flag : 4;      /* all 0 */
    ...
    if (pkt->type == INNO_ITEM_TYPE_SPHERE_POINTCLOUD || pkt->type == INNO_ITEM_TYPE_XYZ_POINTCLOUD) {
      ...
      if (gap == kMemorryFrontGap) {
        *buff = *buff - kMemorryFrontGap;
        ...
        pkt1->common.version.major_version = kInnoMajorVersionDataPacket;
        pkt1->common.version.minor_version = kInnoMinorVersionDataPacket;
  ...
  static bool check_status_packet_v1_and_convert_packet(char **buff, size_t size, int &gap) {
    ...
    if (gap == kMemorryFrontGap) {
      *buff = *buff - kMemorryFrontGap;
      ...
      pkt1->common.version.major_version = kInnoMajorVersionStatusPacket;
      pkt1->common.version.minor_version = kInnoMinorVersionStatusPacket;
