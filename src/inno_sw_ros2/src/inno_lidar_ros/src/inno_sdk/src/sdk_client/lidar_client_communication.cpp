...
int LidarClientCommunication::get_anglehv_table(char *buffer, size_t buffer_len) {
  ...
  config_.copy_from_src(&config_base_);
  int ret =
      http_get(url, response.data(), 60000, &status_code, &content, &content_size, config_.get_conn_timeout_sec);
  if (ret == 0) {
    if (status_code == 200) {
...
const std::string LidarClientCommunication::get_frame_sync_param() {
  char buffer[kSmallBufferSize] = {0};
  int ret = send_get_command_("frame_sync", buffer, sizeof(buffer));
  if (ret != 0) {
    return std::string();
  }
...
const int32_t LidarClientCommunication::set_frame_sync_param(const std::string &param_name) {
  /// server parse need set_frame_sync=value format , so this add frame_sync= prefix
  std::string sendstr = "frame_sync=";
  sendstr += param_name;
  inno_log_debug("sendstr: %s", sendstr.c_str());
  return send_set_command_(sendstr.c_str());
}
